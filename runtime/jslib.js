...
function caml_js_html_entities(s) {
  ...
  return s.replace(/&[^&;]+;/g, function(e) {
    var code = e.substring(1, e.length - 1);
    if(etable[code]) return String.fromCharCode(etable[code]);
    if(code.match(/^#[0-9]+$/))
      return String.fromCharCode(parseInt(code.substring(1)));
    if(code.match(/^#x[0-9]+$/))
      return String.fromCharCode(parseInt('0'+code.substring(1)));
    return e;
  });
}
